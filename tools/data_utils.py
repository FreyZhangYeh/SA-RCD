...
def hist(pixels,name,filename):
    file = os.path.join("/home/zfy/radar-camera-fusion-depth/visl/hist",filename)
    if not os.path.exists(file):
        os.makedirs(file)
    pixels = pixels.squeeze(0).squeeze(0).cpu().numpy()
    pixels = pixels.flatten()
    # pixels = pixels[pixels != 0]
    plt.hist(pixels, bins=100, range=(-0.1, 1.1), color='gray')
...
def save_image(z, path, multiplier=256.0):
    '''
    Saves a depth map to a 16-bit PNG file
