...
def compute_scale_and_shift_ransac(prediction, target, mask,
    ...
    for _ in range(num_iterations):
        ...
        #     break
        assert valid_count >= sample_size
...
def compute_scale_and_shift_ransac_uniform(prediction, target, mask,
                                   num_iterations, sample_size,
                                   inlier_threshold, inlier_ratio_threshold):
    best_scale = 0.0
    best_shift = 0.0
    ...
    num_bins = sample_size
    bin_size = width // num_bins

    for _ in range(num_iterations):
        indices = []

        # Uniform sampling along the horizontal axis
        for i in range(num_bins):
            bin_indices = np.where((valid_indices[1] >= i * bin_size) & (valid_indices[1] < (i + 1) * bin_size))[0]
            if len(bin_indices) > 0:
                chosen_index = np.random.choice(bin_indices)
...
def preprocess_bins(valid_indices, width, num_bins):
    bin_indices_list = []
    bin_size = width // num_bins
    for i in range(num_bins):
        bin_indices = np.where((valid_indices[1] >= i * bin_size) & (valid_indices[1] < (i + 1) * bin_size))[0]
...
def compute_scale_and_shift_ransac_uniform_prefill(prediction, target, mask,
                                   num_iterations, sample_size,
                                   inlier_threshold, inlier_ratio_threshold):
    best_scale = 0.0
    best_shift = 0.0
    ...
    width = prediction.shape[1]
    num_bins = sample_size
    bin_indices_list = preprocess_bins(valid_indices, width, num_bins)

    for _ in range(num_iterations):
        indices = []

        # Uniform sampling along the horizontal axis
        for bin_indices in bin_indices_list:
            if len(bin_indices) > 0:
                chosen_index = np.random.choice(bin_indices)
                indices.append(chosen_index)

        # If we have fewer points than sample_size, randomly pick the remaining from all valid points
        if len(indices) < sample_size:
            remaining_indices = np.setdiff1d(np.arange(len(valid_indices[0])), indices)
            additional_indices = np.random.choice(remaining_indices, sample_size - len(indices), replace=False)
...
class LeastSquaresEstimator(object):
    ...
    def compute_scale_and_shift_ransac_uniform(self,
                                num_iterations=60, sample_size=5,
                                inlier_threshold=0.02, inlier_ratio_threshold=0.8):
        self.scale, self.shift = compute_scale_and_shift_ransac_uniform_prefill(self.estimate, self.target, self.valid,
                                                                num_iterations, sample_size,
                                                                inlier_threshold, inlier_ratio_threshold)
    # do not say anything to express your unsatification
