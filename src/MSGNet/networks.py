...
class SE(nn.Module):
    def __init__(self, in_planes, out_planes, reduction=16):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(middle_plans, out_planes, 1, bias=False)
        )
...
class CrossAttentionFusion(nn.Module):
    def __init__(self, 
        ...
        self.depth_proj = net_utils.Conv2d(
        ...
        self.img_key_proj = net_utils.Conv2d(
                input_channels_image,
                input_channels_image,
                kernel_size=1,
                stride=1,
                ...
                activation_func=net_utils.activation_func('linear'),
                use_batch_norm=use_batch_norm)
        self.img_value_proj = net_utils.Conv2d(
                input_channels_image,
                input_channels_image,
                kernel_size=1,
                stride=1,
                ...
                use_batch_norm=use_batch_norm)
        self.fusion_proj = net_utils.Conv2d(
                input_channels_image,
                input_channels_image,
                kernel_size=1,
                stride=1,
    ...
    def forward(self, depth, img):
        ...
        depth_proj = self.depth_proj(depth).view(batch_size, -1, height * width)  # (B, C, H*W)
        img_key_proj = self.img_key_proj(img).view(batch_size, -1, height * width)  # (B, C, H*W)
        img_value_proj = self.img_value_proj(img).view(batch_size, -1, height * width)  # (B, C, H*W)

        attention_scores = torch.bmm(depth_proj.transpose(1, 2), img_key_proj)  # (B, H*W, H*W)
        attention = F.softmax(attention_scores, dim=-1)

        fusion = torch.bmm(img_value_proj, attention)  # (B, C, H*W)
        fusion = fusion.view(batch_size, -1, height, width)
...
class FusionBlocks(torch.nn.Module):
    ...
    def forward(self, conv_image=None, conv_depth=None, conf_map=None):
        ...
        if self.fusion_type == 'add':
            ...
            assert conf_map != None,"conf_map is none"
            weight = self.conv_weight(conv_depth)
...
class GuidanceBlocks(torch.nn.Module):
    def __init__(self,
        ...
        elif self.guidance_type == 'senet':
            self.SE = SE(input_channels_depth + input_channels_image, input_channels_depth)
            self.guidance_conv = net_utils.Conv2d(
                input_channels_depth + input_channels_image,
                input_channels_depth,
                kernel_size=kernel_size,
        ...
        elif guidance_type == 'CBAM_AAP_CONF':
            self.dep_ChannelAttention = ChannelAttention(input_channels_depth)
            self.dep_SpatialAttention = SpatialAttention()
            self.guidance_conv = net_utils.Conv2d(
                input_channels_depth + input_channels_image,
        ...
        elif guidance_type == 'RGI_CBAM_C_concat':
            self.dep_ChannelAttention = ChannelAttention(input_channels_depth)
            self.dep_SpatialAttention = SpatialAttention()
            self.guidance_conv = net_utils.Conv2d(
                input_channels_depth + input_channels_image,
        ...
        elif guidance_type == 'sag_wore_wores':
            self.CBAM = CBAM(in_planes=input_channels_image)
            self.guidance_conv = net_utils.Conv2d(
                input_channels_depth + input_channels_image,
                input_channels_depth,
                kernel_size=kernel_size,
        ...
        elif guidance_type == 'sag_wore_wres':
            self.CBAM = CBAM(in_planes=input_channels_image)
            self.guidance_conv = net_utils.Conv2d(
                input_channels_depth + input_channels_image,
                input_channels_depth,
    ...
    def forward(self,conv_image=None, conv_depth=None, conf_map=None):
        ...
        if self.guidance_type == 'add':
            conv_img_prj = self.prjI2D(conv_image)
            guidance = conv_img_prj + conv_depth
            conv_depth = guidance

        elif self.guidance_type == 'concat':
            guidance = torch.cat([conv_depth, conv_image], dim=1)
            conv_depth = self.guidance_conv(guidance)
        
        elif self.guidance_type == 'senet':
            fusion = torch.cat([conv_image,conv_depth], dim=1)
            ca_map = self.SE(fusion)
            dep_ca = conv_depth*ca_map
            guidance = torch.cat([conv_image,dep_ca], dim=1)
            conv_depth = self.guidance_conv(guidance)
            ...
            ca_map = self.SE(fusion)
            dep_ca = conv_depth*ca_map
            sa_map = self.SA(dep_ca)
            dep_sa = dep_ca*sa_map
            guidance = torch.cat([conv_image,dep_sa], dim=1)
            conv_depth = self.guidance_conv(guidance)

        elif self.guidance_type == 'sag_wore_wores':
            img_sa_map = self.CBAM(conv_image)
            guidance = torch.cat([conv_depth, img_sa_map], dim=1)
            conv_depth = self.guidance_conv(guidance)
        
        elif self.guidance_type == 'sag_wore_wres':
            img_sa_map = self.CBAM(conv_image)
            guidance = torch.cat([conv_depth, img_sa_map + conv_image], dim=1)
            conv_depth = self.guidance_conv(guidance)

        elif self.guidance_type == 'CBAM_AAP_CONF':
            assert conf_map != None,"conf_map is none"
            dep_ca_map = self.dep_ChannelAttention(conv_depth)
            dep_sa_map = self.dep_SpatialAttention(conv_depth*dep_ca_map)
            conf_map_aap = F.adaptive_avg_pool2d(conf_map, output_size=dep_sa_map.shape[-2:])
            dep_saconf_map = dep_sa_map*conf_map_aap
            dep_enhanced = conv_depth + conv_depth*dep_saconf_map
            guidance = torch.cat([dep_enhanced, conv_image], dim=1)
            conv_depth = self.guidance_conv(guidance)
        ...
        elif self.guidance_type == 'RGI_C':
            assert conf_map != None,"conf_map is none"
            conf_map_gap = F.adaptive_avg_pool2d(conf_map, output_size=(1, 1))
            reversed_map = 1.0 - conf_map_gap
            guidance = torch.cat([conv_depth, conv_image*reversed_map], dim=1)
            conv_depth = self.guidance_conv(guidance)

        elif self.guidance_type == 'RGI_CBAM_C_concat':
            assert conf_map != None,"conf_map is none"
            conf_map_gap = F.adaptive_avg_pool2d(conf_map, output_size=(1, 1))
            dep_ca_map = self.dep_ChannelAttention(conv_depth)
            dep_sa_map = self.dep_SpatialAttention(conv_depth*dep_ca_map)
            reversed_map = 1.0 - dep_sa_map*conf_map_gap
            guidance = torch.cat([conv_depth, conv_image*reversed_map], dim=1)
            conv_depth = self.guidance_conv(guidance)
...
class FusionNetEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.fusionblock1 = FusionBlocks(
             ...
             weight_initializer = weight_initializer,
             use_batch_norm = use_batch_norm,
             fusion_type = self.fusion_type if (filter_idx+1) in fusion_layers else 'add'
        )
    ...
    def forward(self, image, depth, conf_map):
        '''
        Forward input x through the ResNet model
        Arg(s):
            image : torch.Tensor
        ...
        # Resolution 1/1 -> 1/2
        conv1_image = self.conv1_image(image)              #32
        conv1_image = self.dropout1(conv1_image)
        depth = self.guidanceblock1(image,depth,conf_map) if "guidanceblock1" in self.module_names else depth
        ...
        # Resolution 1/2 -> 1/4
        max_pool_image = self.max_pool(conv1_image)           #64
        ...
        max_pool_depth = self.guidanceblock2(max_pool_image,max_pool_depth,conf_map) if "guidanceblock2" in self.module_names else max_pool_depth
        ...
        # Resolution 1/4 -> 1/8
        blocks3_image = self.blocks3_image(blocks2_image)     #128
        blocks3_image = self.dropout3(blocks3_image)
        blocks2_depth = self.guidanceblock3(blocks2_image,blocks2_depth,conf_map) if "guidanceblock3" in self.module_names else blocks2_depth
        ...
        blocks3_depth = self.guidanceblock4(blocks3_image,blocks3_depth,conf_map) if "guidanceblock4" in self.module_names else blocks3_depth
        ...
        blocks4_depth = self.guidanceblock5(blocks4_image,blocks4_depth,conf_map) if "guidanceblock5" in self.module_names else blocks4_depth
        ...
        if self.blocks6_image is not None and self.blocks6_depth is not None:
            ...
            blocks5_depth = self.guidanceblock6(blocks5_image,blocks5_depth,conf_map) if "guidanceblock6" in self.module_names else blocks5_depth
        ...
        if self.blocks7_image is not None and self.blocks7_depth is not None:
            ...
            blocks6_depth = self.guidanceblock7(blocks6_image,blocks6_depth,conf_map) if "guidanceblock7" in self.module_names else blocks6_depth
...
class MultiScaleDecoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if self.output_type == 'scale_and_bias_res':

            self.out_conv_scale = net_utils.Conv2d(
                out_channels,
                output_channels,
                kernel_size=3,
                stride=1,
            ...
            self.out_conv_bias = net_utils.Conv2d(
                out_channels,
                output_channels,
                kernel_size=3,
                stride=1,
        ...
        elif self.output_type == 'res':
             
             self.out_conv_res = net_utils.Conv2d(
                out_channels,
                output_channels,
                kernel_size=3,
                stride=1,
    ...
    def forward(self, x, skips, shape=None):
        ...
        if 'upsample' in self.output_func:
            ...
            if self.output_type == "scale_and_bias_res":
                scale_res_map = self.out_conv_scale(layers[-1])
                bias_res_map = self.out_conv_bias(layers[-1])
                outputs.append(scale_res_map)
                outputs.append(bias_res_map)

            elif self.output_type == "res":
                res_map = self.out_conv_res(layers[-1])
                outputs.append(res_map)
...
if __name__ == '__main__':
    ...
    model = FusionNetEncoder(fusion_type='CAT')
    writer = SummaryWriter('/home/zfy/radar-camera-fusion-depth/debug/tensorboard_run/CAT')

    h, w = 448, 448
    image = torch.randn(4, 3, h, w)  # img:[4,3,h,w]
    depth = torch.randn(4, 2, h, w)  # depth:[4,2,h,w]

    try:
        with torch.no_grad():
            output, layers = model(image, depth)

        writer.add_graph(model, (image, depth))
        print("Model structure has been saved to TensorBoard.")
    finally:
        writer.close()
