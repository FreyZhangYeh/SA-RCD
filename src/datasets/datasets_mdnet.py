...
class StructralNetTrainingDataset(torch.utils.data.Dataset):
    '''
    Dataset for fetching:
        (1) image
        (2) depth
    ...
    def __init__(self,
        ...
        for paths in [ground_truth_paths, lidar_map_paths]:
    ...
    def __getitem__(self, index):
        ...
        # Crop image, depth and adjust intrinsics
        if self.do_random_crop:
            [image,ground_truth, lidar_map] = random_crop(
                inputs=[image,ground_truth, lidar_map],
                shape=self.shape,
                crop_type=self.random_crop_type)

        # Convert to float32
        image, ground_truth, lidar_map = [
            T.astype(np.float32)
            for T in [image, ground_truth, lidar_map]
        ]

        return image, ground_truth, lidar_map

    def __len__(self):
        return self.n_sample
    
class StructralNetInferenceDataset(torch.utils.data.Dataset):
    '''
    Dataset for fetching:
        (1) image
        (2) depth
