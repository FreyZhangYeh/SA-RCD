...
class ConvTransposeBNReLU(nn.Module):
    def __init__(self, ch_in, ch_out, kernel, stride=2, padding=1, output_padding=1,
                 bn=True, relu=True):
...
def param_transfer(fusionnet_dict,structralnet_dict,cuda_visible_devices):
        updated_fusionnet_dict = {}
        encoder = 'image' in list(fusionnet_dict.keys())[0]
        for key, value in structralnet_dict.items():
            if len(cuda_visible_devices) == 1 and 'module.' in key:
                new_key = key.replace('module.', '') 
                first_dot_index = new_key.find(".")
            if encoder:
                new_key = new_key[:first_dot_index] + "_image" + new_key[first_dot_index:]
            #assert new_key in fusionnet_dict, "structralnet_dict's new_key not found in fusionnet_dict"
            if new_key in fusionnet_dict:
                updated_fusionnet_dict[new_key] = value
            else:
                print(f"Key {new_key} not found in the model's state_dict")

        missing_keys = set(fusionnet_dict.keys()) - set(updated_fusionnet_dict.keys())
        if missing_keys:
            print("Missing keys:", missing_keys)
        extra_keys = set(updated_fusionnet_dict.keys()) - set(fusionnet_dict.keys())
        if extra_keys:
            print("Extra keys:", extra_keys)

        return updated_fusionnet_dict
